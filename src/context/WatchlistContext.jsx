...
export function WatchlistProvider({ children }) {
  ...
  const toggleWatchlist = useCallback((show) => {
    ...
    // Freemium gate
    if (!alreadyTracked && watchlist.length >= FREEMIUM_LIMIT && (user?.tier !== 'pro' && user?.tier !== 'premium')) {
  ...
  const atLimit = (user?.tier !== 'pro' && user?.tier !== 'premium') && watchlist.length >= FREEMIUM_LIMIT
