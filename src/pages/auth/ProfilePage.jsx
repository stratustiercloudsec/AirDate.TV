...
// v2.38 mobile fixes: header wraps cleanly, subscription/session rows stack,
// save buttons full-width on mobile, padding responsive
...
// v2.38: full width on mobile, auto on sm+
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <p className="text-white font-bold text-sm">
                {(user?.tier === 'pro' || user?.tier === 'premium') ? 'AirDate Premium' : 'AirDate Free'}
              </p>
          ...
          <Section title="Password" subtitle="Update your password. You'll stay signed in on this device.">
            <form onSubmit={handleChangePassword} className="space-y-4" noValidate>
              ...
              <div>
                ...
                {/* Strength rules — 1-col on mobile, 2-col sm+ */}
                {(pwFocused || newPw) && (
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-1">
                    {PW_RULES.map((rule, i) => (
                      <div key={i} className="flex items-center gap-1.5">
                        ...
                        <span className={`text-[10px] font-bold ${rule.test(newPw) ? 'text-slate-300' : 'text-slate-600'}`}>
                          {rule.label}
                        </span>
                      </div>
                    ))}
